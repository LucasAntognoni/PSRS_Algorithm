...
int main(int argc, char **argv) {
  ...
  for (int iProc = 0; iProc < NumberOfProcs; ++iProc) {
    ...
    if (iProc != Rank) {
      MPI_Send(&NumberOfElementsToSend, 1, MPI_INT, iProc, 0, MPI_COMM_WORLD);
      MPI_Send(Elements + Start, NumberOfElementsToSend, MPI_INT, iProc, 0,
               MPI_COMM_WORLD);
    } else {
      MyStart = Start;
      MyEnd = End;
  ...
  for (int iProc = 0; iProc < NumberOfProcs; ++iProc) {
    ...
    if (iProc != Rank) {
      ...
      // for (int i = 0; i < NumberOfElementsInSublist; ++i) {
      //   printf("%d, ", Sublist[i]);
      // }
